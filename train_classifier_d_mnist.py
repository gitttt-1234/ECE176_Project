...
dirty_mnist_train = ddu_dirty_mnist.DirtyMNIST(".", train=True, download=True, transform=transform_dmnist)
dirty_mnist_test = ddu_dirty_mnist.DirtyMNIST(".", train=False, download=True, transform=transform_dmnist)


loader_train_d_mnist = DataLoader(dirty_mnist_train, batch_size=train_batch_size, num_workers=2,
                          shuffle=True)


loader_test = DataLoader(dirty_mnist_test, batch_size=val_batch_size, num_workers=2)
...
for exp_flag in range(0,2):
    ...
    print("Test Accuracy: on Dirty Mnist")
    test_ac = check_accuracy(loader_test,model)
    print('Accuracy (%.2f)' % (100 * test_ac))
